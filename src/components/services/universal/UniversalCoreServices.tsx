      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
          ...
          {data.cards.map((card, index) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-8">
                <div className={`h-14 w-14 rounded-2xl flex items-center justify-center bg-[#4888E8]/10 text-[#4888E8] group-hover:bg-[#4888E8] group-hover:text-white transition-all duration-500 shadow-sm group-hover:shadow-lg group-hover:shadow-[#4888E8]/30`}>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-[2rem] p-10 border border-slate-100 shadow-lg shadow-[#0F172A]/5 flex items-center gap-8 hover:shadow-xl transition-shadow duration-300">
                ...
                <div>
                    <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-2">{data.footer.left.title}</h4>
                    <p className="text-3xl font-black text-[#0F172A] tracking-tight">{data.footer.left.content}</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2rem] p-10 border border-slate-100 shadow-lg shadow-[#0F172A]/5 flex items-center gap-8 hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="flex-1">
                    <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-4">{data.footer.right.title}</h4>
                    <div className="flex flex-wrap gap-2">
                        {Array.isArray(data.footer.right.content) && data.footer.right.content.map((tag: string, i: number) => (
                            <span key={i} className="px-3 py-1.5 rounded-lg bg-[#FAF7F6] text-[10px] font-black text-slate-600 border border-slate-100 uppercase tracking-wide">
                            {tag}
                            </span>
                        ))}
                    </div>
